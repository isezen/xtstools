...
test_that("mstl2", {
  ...
  attr(x2, "scaled2:center") <- attr(x2, "scaled2:scale") <- NULL
  ...
  attr(x3, "scaled2:center") <- attr(x3, "scaled2:scale") <- NULL
  x3 <- x3 + abs(min(x3)) + 1
  ...
  attr(x4, "scaled2:center") <- attr(x4, "scaled2:scale") <- NULL
  ...
  attr(x5, "scaled2:center") <- attr(x5, "scaled2:scale") <- NULL
  x5 <- x5 + abs(min(x5)) + 1

  # additive
  s10 <- mstl2(x5, s.window = "periodic")
  expect_equal(x5, Reduce("+", as.list(s10[,2:4])))

  # multiplicative
  s11 <- mstl2(x5, s.window = "periodic", type = "m")
